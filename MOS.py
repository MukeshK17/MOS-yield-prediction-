...
for j in range(len(y_vals)):
    for i in range(x_vals_plot.shape[0]):
        ...
        sigma = (((sigma_x_p**2)- (sigma_x_p*sigma_y_p) + (sigma_y_p**2)  )**0.5)  #+ (3*(tau_xy_p**2))
        # sigma = ((sigma_x**2 + 1.5*(T**2))**0.5)  #+ (3*(tau_xy_p**2))
        # sigma = (sigma_xx)
        sigmas.append(sigma)
...
if st.button("Calculate"):
    ...
    st.pyplot(plt)
        ...
        # Count number of elements exceeding yield strength (von Mises and Tresca)
    sigmas_np = np.array(sigmas)
    tresca_np = np.array(Tresca)

    num_vm_exceed = np.sum(sigmas_np >= Sigma_yield)
    num_tresca_exceed = np.sum(tresca_np >= (Sigma_yield / 2))

    st.write(f"### Yield Prediction Summary:")
    st.write(f"- Number of points exceeding yield (von Mises): {num_vm_exceed}")
    st.write(f"- Number of points exceeding yield (Tresca): {num_tresca_exceed}")


    # st.write(len(Tresca > = Sigma_yield))
    # st.write("Number of elements exceeding yield strength (Tresca):", np.sum(Tresca >= Sigma_yield))
